...
|*{chart-version}* |{original-repo-url}/subpath/to/the/chart[Chart] |{original-repo-url}//subpath/to/the/values.yaml[`values.yaml`] (here you can also use the artifacthub.io)
|===
...
_This section will contain the more technical details of the module. It is mainly generated automatically by Terraform Docs (everything that is between the comments `// BEGIN_TF_DOCS` and `// END_TF_DOCS`). However, it is recommended to, at the bare minimum, add a dependencies section to explain the reason on why this module could depend on another one from the DevOps Stack._
