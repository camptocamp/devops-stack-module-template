...
_This section will contain the more technical details of the module. It is mainly generated automatically by Terraform Docs (everything that is between the comments `// BEGIN_TF_DOCS
=== Requirements
...
// END_TF_DOCS`). However, it is recommended to, at the bare minimum, add a dependencies section to explain the reason on why this module could depend on another one from the DevOps Stack._

_You can safely delete the content between the comments `// BEGIN_TF_DOCS` / `// END_TF_DOCS` and `// BEGIN_TF_TABLES
= Requirements
...
|===
// END_TF_TABLES` when creating your on module._
